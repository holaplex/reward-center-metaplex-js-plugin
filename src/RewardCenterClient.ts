...
} from "./operations/acceptOffer";
import { BuyInput, buyOperation } from "./operations/buy";
import {
  CancelOfferInput,
  cancelOfferOperation,
} from "./operations/cancelOffer";
import {
  CloseListingInput,
  closeListingOperation,
} from "./operations/closeListing";
import {
  CreateListingInput,
...
} from "./operations/updateListing";
import {
  UpdateOfferInput,
  updateOfferOperation,
} from "./operations/updateOffer";
...
export class RewardCenterClient {
  ...
  updateListing(input: UpdateListingInput, options?: OperationOptions) {
    return this.metaplex
      .operations()
      .execute(updateListingOperation(input), options);
  }

  closeListing(input: CloseListingInput, options?: OperationOptions) {
    return this.metaplex
      .operations()
      .execute(closeListingOperation(input), options);
  }
}
